...
int TimeStep = 0;             // Global timestep of the simulation (resets after F12 is pressed)
...
int client_fd;         // socket file descriptor for connection with the connector
int steps_to_advance = 0;

// Allocate memory for the structure received through the socket
double **NeighborsFromNS; // This could (should?) be optimized because we allocate a lot of memory for values that could fit in a few bytes.
double **LinkQualityFromNS;
...
void Initialize()
{
    ...
    // Allocate memory for neighbor matrices and linkQUality matrices (global variables)
    NeighborsFromNS = malloc(ActualSitParams.NumberOfAgents * sizeof(double *));
    LinkQualityFromNS = malloc(ActualSitParams.NumberOfAgents * sizeof(double *));
    for (i = 0; i < ActualSitParams.NumberOfAgents; i++)
    {
        NeighborsFromNS[i] = malloc(ActualSitParams.NumberOfAgents * sizeof(double));
        LinkQualityFromNS[i] = malloc(ActualSitParams.NumberOfAgents * sizeof(double));
    }
    ...
    /* TODO: Why do we initialize the positions and velocities of the agents again ? done with InitCond() line 159*/
...
void DrawCopters(phase_t *Phase, phase_t *GPSPhase, const int TimeStep)
{
                ...
                if (ActualVizParams.DisplayPressure == true)
                {
                    DrawGradientColoredCircle(RealToGlCoord_2D(AgentsCoordinates[0] -
                                              ...
                                              RealToGlCoord_2D(AgentsCoordinates[1] -
                                                                   ActualVizParams.CenterY,
                                                               ActualVizParams.MapSizeXY),
                                              RealToGlCoord_2D(3 * Phase->Pressure[i],
                                                               ActualVizParams.MapSizeXY),
                                              RealToGlCoord_2D(100, ActualVizParams.MapSizeXY), red, gr, 25);
                }
            }
        }
    ...
    else
    {
        ...
        for (i = -HowManyTics; i < HowManyTics; i++)
        {
            ...
            glVertex3d(i * TicDensity, LengthOfAxis, RealToGlCoord_3D(-1000.0, ActualVizParams.MapSizeXY));
        }
        for (i = -HowManyTics; i < HowManyTics; i++)
        {
            ...
            glVertex3d(-HowManyTics * TicDensity, i * TicDensity,
                       RealToGlCoord_3D(-1000.0, ActualVizParams.MapSizeXY));
            glVertex3d(LengthOfAxis, i * TicDensity, RealToGlCoord_3D(-1000.0, ActualVizParams.MapSizeXY));
        }
        glEnd();
...
void DisplayTrajs()
{
    ...
    RealSpeed =
        RealSpeed * 0.9 + 0.1 * (TimeStep - LastTimeStep) * ActualSitParams.DeltaT / (NowTime.tv_sec - LastTime.tv_sec + (NowTime.tv_usec - LastTime.tv_usec) / 1e6);
    LastTime = NowTime;
    ...
    if (ActualVizParams.CoMFollowing == true && ActualVizParams.TwoDimViz == true)
    {
...
void UpdatePositionsToDisplay()
{
    ...
    if (ActualVizParams.Paused == false)
    {
        ...
        // printf("Positions:\n");
        // PrintMatrix(PhaseData[Now].Coordinates, ActualSitParams.NumberOfAgents, 3);

#ifdef COSIM_MODE
        /** synchronize with the co-simulator every "steps_to_advance" loops, 
         * where steps_to_advance is configured by the last received value on the socket
         * Structure of the message: 
         * [ t, neigh[0][0], neigh[0][1], ... , neigh[0][N-1], PL[0][0], PL[0][1], ... PL[0][N-1], neigh[1][0], ... , neigh[N-1][N-1], ... , PL[N-1][N-1] ]
         * Where N is the number of agents, 
         * neigh[i][j] are the IDs of the neighbors ordered by increasing Pathloss, and 
         * PL[i][j] are the pathloss values for the corresponding agents (PL[i][j] is the pathloss between agent i and agent neigh[i][j]) */
        if (steps_to_advance <= 0)
        {
            double buf[1024];
            receiveOneMessageDoubleVector(client_fd, buf); // blocking line, waits to receive a message on the UDS socket.
            int t = (int)buf[0];                 // value t (in seconds) that should be advanced by the simulator
            // printf("received: %f\n", buf[0]);
            // Verify that t is divisible by the "sped up" delta_t.
            if (t % (int)(ActualSitParams.DeltaT * ActualVizParams.VizSpeedUp * 1000000) == 0)
            {
                steps_to_advance = (int)(t / (int)(ActualSitParams.DeltaT * ActualVizParams.VizSpeedUp * 1000000));
            }
            else
            {
                printf("Error: The connector asked to advance %d us but the minimal time-step (delta_t * VizSpeedUp) is %d us.\n", t, (int)(ActualSitParams.DeltaT * ActualVizParams.VizSpeedUp * 1000000));
                exit(-1);
            }
            ...
            for (i = 0 ; i < ActualSitParams.NumberOfAgents; i++)
            {
                ...
                for (j = 0; j < ActualSitParams.NumberOfAgents - 1; j++)
                {
                    ...
                    LinkQualityFromNS[i][j+1] = buf[1 + i * 2 * ActualSitParams.NumberOfAgents + 2*j + 1];
                    // printf("%f -> %f | ", NeighborsFromNS[i][j+1], LinkQualityFromNS[i][j+1]);
                }
                // printf("\n");
            }
        }
        ...
        for (i = 0; i < ActualVizParams.VizSpeedUp; i++)
        {
            if (Now < TimeStepsToStore - 1)
            {
                ...
                Step(&ActualPhase, &GPSPhase, &GPSDelayedPhase,
                     ...
                     WindVelocityVector, Accelerations, TargetsArray, Polygons, &Hull, Verbose, NeighborsFromNS, LinkQualityFromNS);
        ...
        /* send response to the connector if we executed the right number of steps */
        if (steps_to_advance <= 0)
        {
            char *buffer;
...
void HandleKeyBoard(unsigned char key, int x, int y)
{
    ...
    else if (key == 'z')
    {
        ...
        if (ActualVizParams.UnitParamsDisplayed == true && ActualVizParams.WhichParamIsSelected > 17)
        {

            ActualVizParams.WhichParamIsSelected = 17;
        }
        else if (ActualVizParams.FlockingParamsDisplayed == true && ActualVizParams.WhichParamIsSelected >
    ...
    else
    {
        ...
        else if (ActualVizParams.FlockingParamsDisplayed == true)
        {
            ...
            if (keyint < ActualFlockingParams.NumberOfParameters + 1 && keyint > 0 && ActualFlockingParams.Params[keyint - 1].InMenu == true)
            {
                ActualVizParams.WhichParamIsSelected = keyint;
            }
        }
...
void HandleKeyBoardSpecial(int key, int x, int y)
{
    ...
    else if (key == GLUT_KEY_LEFT && ActualVizParams.CamTraj == false)
    {
        ...
        if (Modder == GLUT_ACTIVE_ALT && ActualVizParams.WhichAgentIsSelected != 0 && ActualVizParams.CoMFollowing == false)
        {
            ActualVizParams.WhichAgentIsSelected -= 1;
        }
        else if (Modder == GLUT_ACTIVE_ALT && ActualVizParams.WhichAgentIsSelected == 0 && ActualVizParams.CoMFollowing == false)
        {
            ActualVizParams.WhichAgentIsSelected =
                ActualSitParams.NumberOfAgents;
        }
    ...
    {
        if (Modder == GLUT_ACTIVE_ALT && ActualVizParams.WhichAgentIsSelected != ActualSitParams.NumberOfAgents && ActualVizParams.CoMFollowing == false)
        {
            ActualVizParams.WhichAgentIsSelected += 1;
        }
        else if (Modder == GLUT_ACTIVE_ALT && ActualVizParams.WhichAgentIsSelected == ActualSitParams.NumberOfAgents && ActualVizParams.CoMFollowing == false)
        {
    ...
    else if (ActualVizParams.UnitParamsDisplayed == true)
    {
        ...
        if (ActualVizParams.WhichParamIsSelected == 7 || ActualVizParams.WhichParamIsSelected == 8)
        {
...
void HandleMouse(int button, int state, int x, int y)
{
    ...
    if (ActualVizParams.TwoDimViz == true)
    {
        ...
        else if (button == 2 && state == GLUT_DOWN && ActualVizParams.CoMFollowing == false)
        {
            ActualVizParams.WhichAgentIsSelected =
                ActualSitParams.NumberOfAgents;
            ActualVizParams.CenterX +=
...
void print_help(void)
{
    printf("This is robotsim created at ELTE Department of Biological Physics.\n"
           ...
           "-outputconf FILE    defines an input file for output configuration file\n"
           "\n");
}
...
int main(int argc, char *argv[])
{
    ...
    /* Setting up structures containing the parameters of the flocking model
    ...
    /* Number of steps that need to be stored */
    ...
    else
    {
        ...
        if (FALSE != ActualSaveModes.SaveDistanceBetweenUnits)
        {
            ...
            if (STAT == ActualSaveModes.SaveDistanceBetweenUnits || STEADYSTAT == ActualSaveModes.SaveDistanceBetweenUnits)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveDistanceBetweenNeighbours)
        {
            ...
            if (STAT == ActualSaveModes.SaveDistanceBetweenNeighbours || STEADYSTAT ==
        ...
        if (FALSE != ActualSaveModes.SaveVelocity)
        {
            ...
            if (STAT == ActualSaveModes.SaveVelocity || STEADYSTAT == ActualSaveModes.SaveVelocity)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveCoM)
        {
            ...
            if (STAT == ActualSaveModes.SaveCoM || STEADYSTAT == ActualSaveModes.SaveCoM)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveCorrelation)
        {
            ...
            if (STAT == ActualSaveModes.SaveCorrelation || STEADYSTAT == ActualSaveModes.SaveCorrelation)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveHullArea)
        {
            ...
            if (STAT == ActualSaveModes.SaveHullArea || STEADYSTAT == ActualSaveModes.SaveHullArea)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveCollisionRatio)
        {
            ...
            if (STAT == ActualSaveModes.SaveCollisionRatio || STEADYSTAT == ActualSaveModes.SaveCollisionRatio)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveAcceleration)
        {
            ...
            if (STAT == ActualSaveModes.SaveAcceleration || STEADYSTAT == ActualSaveModes.SaveAcceleration)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SaveReceivedPowers)
        {
            ...
            if (STAT == ActualSaveModes.SaveReceivedPowers || STEADYSTAT == ActualSaveModes.SaveReceivedPowers)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        if (FALSE != ActualSaveModes.SavePressure)
        {
            ...
            if (STAT == ActualSaveModes.SavePressure || STEADYSTAT == ActualSaveModes.SavePressure)
            {
                strcpy(OutputFileName, ActualStatUtils.OutputDirectory);
        ...
        while (ActualStatUtils.ElapsedTime < ActualSitParams.Length && ActualVizParams.ExperimentOver == false)
        {
        ...
        if (FALSE != ActualSaveModes.SaveCollisionRatio)
        {
            ...
            temp_time = (STEADYSTAT == ActualSaveModes.SaveCollisionRatio ? ActualStatUtils.ElapsedTime -
                                                                                ActualSitParams.StartOfSteadyState
                                                                          : ActualStatUtils.ElapsedTime);
