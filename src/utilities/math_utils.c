...
void SortEigenvaluesEigenvectors(double *Eigenvalues, double **Eigenvectors, int Length)
{
...
int comp_double(const void *x, const void *y)
{
...
/* Returns the index of the first occurrence of a value in a int array */
...
double InnerSumDouble(double *InputVector, int Dim)
{
 ...
 */
void PrintMatrix(double **matrix, int numberOfLines, int numberOfColumns)
{
    int row, columns;
    for (row = 0; row < numberOfLines; row++)
    {
        ...
        {
            printf("%3.3f\t", matrix[row][columns]);
        }
        printf("\n");
    }
}
                                  ...
                                  double *Axis, const double Angle)
{
    ...
    OutputVector[0] =
        (cosAngle +
         AxisUnitVect[0] * AxisUnitVect[0] * (1 - cosAngle)) *
            Temp[0] +
        (AxisUnitVect[0] * AxisUnitVect[1] * (1 - cosAngle) -
         AxisUnitVect[2] * sinAngle) *
            Temp[1] +
        (AxisUnitVect[0] * AxisUnitVect[2] * (1 - cosAngle) +
         AxisUnitVect[1] * sinAngle) *
            Temp[2];
    OutputVector[1] =
        (AxisUnitVect[0] * AxisUnitVect[1] * (1 - cosAngle) +
         AxisUnitVect[2] * sinAngle) *
        ...
        (cosAngle +
         AxisUnitVect[1] *
             AxisUnitVect[1] * (1 - cosAngle)) *
            Temp[1] +
        (AxisUnitVect[1] * AxisUnitVect[2] * (1 - cosAngle) -
         AxisUnitVect[0] * sinAngle) *
            Temp[2];
    OutputVector[2] =
        ...
        (AxisUnitVect[2] * AxisUnitVect[1] * (1 - cosAngle) +
         AxisUnitVect[0] * sinAngle) *
            Temp[1] +
        (cosAngle +
         AxisUnitVect[2] * AxisUnitVect[2] * (1 - cosAngle)) *
            Temp[2];
}
...
double DistanceOfTwoPoints3D(double *P1, double *P2)
{
...
int AtShadow(double *x1, double *x2, double *WhichPoint)
{
    ...
    // in shadow
    if ((ScalarProduct(Temp1, Temp4, 2) >= 0) && (ScalarProduct(Temp2, Temp3, 2) >= 0))
    {
        ...
        if (WhichPoint[1] > min)
        {
            if (WhichPoint[1] <= max)
            {
                ...
                if (WhichPoint[0] <= max)
                {
                    if (p1y != p2y)
                    {
                        xints = (WhichPoint[1] - p1y) * (p2x - p1x) / (p2y - p1y) + p1x;
                    }
                     ...
                     double *Point, double *CentreOfCircle, const double Radius)
{
    ...
    if (d < Radius)
    { // What if "Point" is inside the circle?
        return 0;
    }
    else if (d > Radius)
    {
    ...
    { // What if "Point" is on the circle?
    ...
    else
    {
        ...
        Intersection[0] =
            ((RefPointB1[0] - RefPointB2[0]) * temp_coeff1 -
             (RefPointA1[0] - RefPointA2[0]) * temp_coeff2) /
            det;
        Intersection[0] =
            ((RefPointB1[1] - RefPointB2[1]) * temp_coeff1 -
             (RefPointA1[1] - RefPointA2[1]) * temp_coeff2) /
            det;
        return 0.0;
    }
}
...
double AdjacencyPotentialFunction(double d, const double r1, const double r2, const double R, const double mu1, const double mu2)
{

    if (d < r1)
    {
        return mu1 / (pow(d, 2)) - mu1 / (pow(r1, 2));
    }
    else if (d < r2)
    {
        return 0.0;
    ...
    {
        return mu2 / (pow(R, 2) - pow(d, 2)) - mu2 / (pow(R, 2) - pow(r2, 2));
    }
}

double AdjacencyPotentialGradient(double d, const double R, const double r1, const double r2, const double mu1, const double mu2)
{
    if (d < r1)
    {
        return (-2 * mu1 * d) / (pow(d, 4));
    }
    else if (d < r2)
    {
        return 0.0;
    ...
    {
        return (2 * mu2 * d) / pow(pow(R, 2) - pow(d, 2), 2);
    }
}

double sat(double x, double xi)
{
    if (x > xi)
        return 1.0;
    if (x < -xi)
