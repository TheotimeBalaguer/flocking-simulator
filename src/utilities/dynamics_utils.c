                   ...
                   const int NumberOfInnerStates, const int Resolution)
{
    ...
    Phase->SecondEigenvalue = 0.0;
    Phase->SecondEigenvector = doubleVector(NumberOfAgents);
    Phase->Pressure = doubleVector(NumberOfAgents);
    Phase->ReceivedPower = doubleMatrix(NumberOfAgents, NumberOfAgents);
...
void freePhase(phase_t *Phase, const int Resolution)
{
    ...
    freeMatrix(Phase->ReceivedPower, Phase->NumberOfAgents, Phase->NumberOfAgents);
                              ...
                              int *ExceptWhichAgents, const double NeighbourhoodRange)
{
    ...
    for (j = 0; j < Phase->NumberOfAgents; j++)
    {
        ...
        if (VectAbs(CoordinateDifference) <= NeighbourhoodRange && IsItAnException == false)
        {
            for (i = 0; i < 3; i++)
            {
                   ...
                   const int HowManyRowsToSave)
{
    ...
    for (i = 0; i < HowManyRowsToSave; i++)
    {
        for (j = 0; j < PhaseData[0].NumberOfAgents; j++)
        {
            ...
            {
                PhaseData[i].Laplacian[j][k] = PhaseData[HowManyRows - HowManyRowsToSave + i].Laplacian[j][k];
            }
                 ...
                 const double ArenaSize)
{
    ...
    for (int xOffset = -numberOfSigma * sigma; xOffset < numberOfSigma * sigma; xOffset += SquareSize)
    {
        for (int yOffset = -numberOfSigma * sigma; yOffset < numberOfSigma * sigma; yOffset += SquareSize)
        {
            ...
            if (xOffset * xOffset + yOffset * yOffset > numberOfSigma * numberOfSigma * sigma * sigma)
                continue;
                          ...
                          const int fromAgent, const int toAgent, const double RadiusOfCopter)
{
    ...
    for (i = fromAgent; i < toAgent; i++)
    {
        ...
        while (false == IsArrangementCorrect)
        {
            ...
            FillVect(AgentsCoordinates,
                     XCenter + randomizeDouble(-XSize / 2.0, XSize / 2.0),
                     YCenter, ZCenter + randomizeDouble(-ZSize / 2.0, ZSize / 2.0));
                      ...
                      const bool CountCollisions, const double RadiusOfCopter)
{
    ...
    if (CountCollisions == true)
    {
        for (j = 0; j < ActualPhase->NumberOfAgents; j++)
        {
            ...
            Collisions += (PreviousSituation == false && AgentsInDanger[j] == true);
        }
    }
...
void SwapAgents(phase_t *Phase, const int i, const int j)
{
    ...
    /* Received power */
    temp_pointer = Phase->ReceivedPower[i];
    Phase->ReceivedPower[i] = Phase->ReceivedPower[j];
    Phase->ReceivedPower[j] = temp_pointer;
...
int OrderAgentsByPower(phase_t *Phase, int SizeToSort, int WhichAgent)
{
    ...
    for (i = 1; i < SizeToSort; i++)
    {
        ...
        RP1 = Phase->ReceivedPower[WhichAgent][i];

        j = i;

        RP2 = Phase->ReceivedPower[WhichAgent][j - 1];
        ...
        while (j > 0 && RP2 < RP1)
        {
            ...
            RP1 = Phase->ReceivedPower[WhichAgent][j];
            RP2 = Phase->ReceivedPower[WhichAgent][j - 1];
        }
    }
                              ...
                              const int TrueAgent, const double packet_loss)
{
    ...
    for (i = Phase->NumberOfAgents - 1; i >= NumberOfNearbyAgents; i--)
    {
        ...
        Pow = Phase->ReceivedPower[TrueAgent][i];
        ...
        case 0:
            ...
            break;
        case 1:
            if (Dist != 0 && Phase->ReceivedPower[TrueAgent][i] > power_thresh && !packet_loss_rand)
            {
                SwapAgents(Phase, i, NumberOfNearbyAgents);
                NumberOfNearbyAgents++;
                i++;
...
double DegradedPower(const double Dist, double DistObst, double Loss, unit_model_params_t *UnitParams)
{
    ...
    {
        if (Dist < UnitParams->ref_distance.Value && Dist != 0)
        { // Remember that all measured distances are in cm so Ref_dist should be in cm too
            Power = UnitParams->transmit_power.Value - (10 * UnitParams->gamma.Value *
                                                            log10((UnitParams->ref_distance.Value - DistObst) * 0.01 * UnitParams->freq.Value) +
                                                        32.44 + Loss + randomizeGaussDouble(0, 2));
        }
        else if (Dist == 0)
        {
            Power = 0;
        ...
        {
            Power = UnitParams->transmit_power.Value - (10 * UnitParams->gamma.Value *
                                                            log10((Dist - DistObst) * 0.01 * UnitParams->freq.Value) +
                                                        32.44 + Loss + randomizeGaussDouble(0, 2)); // c en m.GHz, dist in meters, freq in GHz (see Friis model)
        }
    }
    else
    {
        ...
        { // Remember that all measured distances are in cm so Ref_dist should be in cm too
            Power = UnitParams->transmit_power.Value - (10 * UnitParams->gamma.Value *
                                                            log10(UnitParams->ref_distance.Value * 0.01 * UnitParams->freq.Value) +
                                                        32.44 + randomizeGaussDouble(0, 2));
        }
        else
        {
            Power = UnitParams->transmit_power.Value - (10 * UnitParams->gamma.Value *
                                                            log10(Dist * 0.01 * UnitParams->freq.Value) +
                                                        32.44 + randomizeGaussDouble(0, 2)); // c en m.GHz, dist in meters, freq in GHz (see Friis model)
        }
    }
    return Power;
}
...
/**
 * Compute the received power based on the Friis Propagation model with obstacles
 * @param communication_type: 2 = obstacles taken into account
 * @param Dist: distance between the two agents (m)
 * @param DistObst: distance between the two agents and the obstacle (m)
 * @param DistRef: reference distance (m)
 * @param Pt: transmit power (dB)
 * @param Gamma: Coef for air
 * @param GammaObst: Coef for obstacle
 * @param freq: frequency (Hz)
 * @return: the received power (dB)
 */
double ComputeReceivedPower(const int communication_type, const double Dist, const double DistObst, const double DistRef,
                            const double Pt, const double Gamma, const double GammaObst,
                            const double freq)
{
    if (Dist == 0.0)
    {
    ...
    double newDist = Dist;
    double Pr = 0;
    // TODO: [optim] PL0 is computed every time but is global, compute it only one time.
    double PL0 = 10 * Gamma * log10(DistRef) + 10 * Gamma * log10(freq) - 147.55;

    double PObst = 0;
    if (communication_type == 2) // Obstacles are taken into account
    {
        newDist = Dist - DistObst;
        if (DistObst > 1) // We consider obstacles from 1 m thick
        {
            PObst = 10 * GammaObst * log10(DistObst);
        }
    }

    if (newDist >= DistRef)
    {
        Pr = Pt - PL0 - 10 * Gamma * log10(newDist / DistRef); // - randomizeGaussDouble(0, 2);
    }
    else
    {
        Pr = Pt - PL0;
...
void ConstructWeightedLaplacianMatrixDistance(double **OutputMatrix, phase_t *Phase, int NumberOfAgents, double R) 
{
    int i, j;
    double theta = sqrt(pow(R, 2) / (2.0 * log(2.0)));

    // Compute adjacency matrix
    double **AdjacencyMatrix = doubleMatrix(NumberOfAgents, NumberOfAgents);
    for (i = 0; i < NumberOfAgents; i++)
    {
        // Phase->NeighSet[i][0] == i so skip j = 0
        for (j = 1; j < NumberOfAgents; j++)
        {
            int neighborID = Phase->NeighSet[i][j];
            if (neighborID != -1)
            {
                double distance = DistanceOfTwoPoints3D(Phase->Coordinates[i], Phase->Coordinates[neighborID]);
                AdjacencyMatrix[i][neighborID] = exp((pow(R, 2) - pow(distance, 2)) / (2 * pow(theta, 2)));
            }
        }
    }

    // Compute the degree matrix
    double **DegreeMatrix = doubleMatrix(NumberOfAgents, NumberOfAgents);
    for (i = 0; i < NumberOfAgents; i++)
    {
        DegreeMatrix[i][i] = InnerSumDouble(AdjacencyMatrix[i], NumberOfAgents);
    }
    ...
    for (i = 0; i < NumberOfAgents; i++)
    {
        for (j = 0; j < NumberOfAgents; j++)
        {
            OutputMatrix[i][j] = DegreeMatrix[i][j] - AdjacencyMatrix[i][j];
        }
    }
}

void ConstructWeightedLaplacianMatrixPower(double **OutputMatrix, phase_t *Phase, int NumberOfAgents, double ComThreshold)
{
    int i, j;
    double theta = sqrt(pow(ComThreshold, 2) / (2.0 * log(2.0)));
    ...
    for (i = 0; i < NumberOfAgents; i++)
    {
        ...
        for (j = 1; j < NumberOfAgents; j++)
        {
            ...
            if (neighborID != -1)
            {
                double power = Phase->ReceivedPower[i][neighborID];
                AdjacencyMatrix[i][neighborID] = exp((pow(ComThreshold, 2) - pow(power, 2)) / (2 * pow(theta, 2)));
            }
        }
    }
    // Compute the degree matrix
...
double *flatten(double **array, int rows, int cols)
{
    // Allocate memory for the 1D array
    double *flat_array = (double *)malloc(rows * cols * sizeof(double));
    if (flat_array == NULL)
    {
...
// Function to expand a 1D array into a 2D array
double **expand(double *flat_array, int rows, int cols)
{
...
/* Quick and dirty to print InnerState */
void PrintInnerState(double *AgentInnerState)
{
    printf("Attraction: (%f, %f)\n", AgentInnerState[5], AgentInnerState[6]);
    printf("Slip: (%f, %f)\n", AgentInnerState[7], AgentInnerState[8]);
    printf("Obstacle: (%f, %f)\n", AgentInnerState[9], AgentInnerState[10]);
    printf("Normalized Vel.: (%f, %f)\n", AgentInnerState[11], AgentInnerState[12]);
    printf("Potential Flocking: (%f, %f)\n", AgentInnerState[13], AgentInnerState[14]);
    printf("Connectivity Control: (%f, %f)\n\n", AgentInnerState[15], AgentInnerState[16]);
}

/**
 * Take an initial phase with the right NumberOfAgents and ReceivedPower complete for the WhichAgent agent, and
 * sort the neighbors by decreasing ReceivedPower.
 * "RealIDs" is update (sorted by descending receive power)
 * "ReceivePower[0]" is updated (sorted descending order)
 * "NeighSet[0]" is updated (neighbours sorted, -1 otherwise)
 * @param Phase The phase to sort 
 * @param WhichAgent The ego agent
 * @param SizeToSort The number of neighbors to sort
*/
void DefineNeighborhood(phase_t *Phase, const int WhichAgent, const int SizeToSort, const double ReceptionThreshold, const int size_neighborhood)
{
    ...
    SwapAgents(Phase, WhichAgent, 0);
    double tempPow = Phase->ReceivedPower[0][0];
    Phase->ReceivedPower[0][0] = Phase->ReceivedPower[0][WhichAgent];
    Phase->ReceivedPower[0][WhichAgent] = tempPow;

    for (i=0; i<SizeToSort; i++) {
        Phase->NeighSet[0][i] = -1;
    }
    
    // Order neighbors by decreasing received power
    for (i = 1; i < SizeToSort; i++)
    {
        for (j = i + 1; j < SizeToSort; j++)
        {
            if (Phase->ReceivedPower[0][i] < Phase->ReceivedPower[0][j])
            {
                SwapAgents(Phase, i, j);

                double tempPow = Phase->ReceivedPower[0][i];
                Phase->ReceivedPower[0][i] = Phase->ReceivedPower[0][j];
                Phase->ReceivedPower[0][j] = tempPow;
            }
        }
    }
